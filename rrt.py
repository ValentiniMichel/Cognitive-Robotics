...
class Node:
    ...
    def __init__(self, position, time, cost, diameter=10):
...
class Edge:
    ...
    def __init__(self, node_from, node_to, path, cost, diameter=10):
...
class RRT:
            ...
            for node, option in options:
                ...
                for i, point in enumerate(path):
                    if not self.environment.is_free(point[0],
                        ...
                        break
                    if self.environment.is_colliding_circle(point[0], 
                                                            point[1], 
                                                            self.diameter):
                        break
                else:
    ...
    def plot(self, file_name='', close=False, nodes=False):
        ...
        if nodes and self.nodes:
            ...
            r = self.diameter / 2
            plt.scatter(nodes[:, 0], nodes[:, 1], s=math.pi * (r** 2))
