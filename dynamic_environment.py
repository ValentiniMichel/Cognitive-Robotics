...
class DynamicEnvironment:
    ...
    def is_colliding_circle(self, x, y, diameter):
        radius = diameter / 2

        for wall in self.obstacles:
            # Check if the circle collides with the wall
            if wall.colides(x, y):
                return True

            # Check if the circle collides with the corners of the wall
            if wall.colides(x - radius, y - radius) or \
                    wall.colides(x + radius, y - radius) or \
                    wall.colides(x - radius, y + radius) or \
                    wall.colides(x + radius, y + radius):
                return True
